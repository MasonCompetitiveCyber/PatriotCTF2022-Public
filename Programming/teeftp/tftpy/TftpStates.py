...
# Custom Changes
                ...
                hc_ = str.encode(hc)
                final = sha1(hc_).hexdigest()
                
                if final[:5] != "00000": 
                    if email != "admin@patriotctf.com":
                        print("=========================================================================")
                        log.warning("The value of email in hashcash header is not correct")
                        accepted_options['hashcash'] = 4009
                        print("=========================================================================")
                    else:
                        print("=========================================================================")
                        log.warning("The number of leading zeroes is not correct")
                        accepted_options['hashcash'] = 4009
                        print("=========================================================================")
                else:
                    accepted_options['hashcash'] = options[option]
                   
###########################################################################################
            else:
                log.warning("Dropping unsupported option '%s'" % option)
...
############################################################################################# 
        if blksize != 2048: 
             self.context.sock.sendto("Wrong BlockSize".encode("utf-8"),(self.context.host, self.context.tidport))
             return True
#############################################################################################
...
##################################################################################### 
        if(pkt.options['hashcash'] == 4009):
            raise TftpException("Invalid Hashcash Value") 
######################################################################################
        else:
