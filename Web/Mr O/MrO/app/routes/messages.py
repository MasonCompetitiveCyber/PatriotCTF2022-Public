...
def messages():
    if request.method == 'GET':
        ...
        if "username" in request.form:
            ...
            elif "<" in message or ">" in message or "|" in message or "[" in message or "]" in message or ";" in message:
                flash('Hey. No funny business. These characters are not allowed: <>[]|;', 'danger')
                return redirect(url_for('home'))
            elif "{{" in message or "}}" in message or "config" in message:
                flash('I got hacked by someone using those characters, not happening again! >:(', 'danger')
                return redirect(url_for('home'))
            if msg_to == 'Send to:':
